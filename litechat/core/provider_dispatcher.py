...
class ProviderDispatcher:
    ...
    def chat(self, convo: Conversation, new_user_message: str, streaming: bool = False, on_chunk=None) -> str:
        client = self.clients.get(convo.provider_id)
        if client is None:
            raise ValueError(f"No client configured for provider '{convo.provider_id}'")
        return client.chat(convo, new_user_message, streaming=streaming, on_chunk=on_chunk)
