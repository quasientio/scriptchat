...
def import_conversation_from_file(path: Path, conversations_root: Path) -> Conversation:
    ...
    Args:
        path: Path to exported file (.json or .md)
        conversations_root: Root directory to store imported conversation
    ...
    if desired_id:
        ...
        if dir_name != desired_id:
            # collision; fall back to creating new directory name using save_name semantics
...
def _conversation_from_json_export(path: Path) -> tuple[Conversation, Optional[str]]:
    ...
    for msg in data.get("messages", []):
        ...
        if role in ("user", "assistant"):
            ...
            # keep inline system messages too
            messages.append(Message(role=role, content=content))
